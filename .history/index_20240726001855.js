...
const Designation = require('./modles/UserType'); // Adjust the path to your UserType model
...
app.put("/designations/:id", async (req, res) => {
  ...
  const index = designations.findIndex((data) => data.id === parseInt(id));
  if (index !== -1) {
    designations[index] = updatedDesignation; // In-memory update
  }
  const designation = await Designation.findOneAndUpdate({ id: parseInt(id) }, updatedDesignation, { new: true });
  if (!designation) {
...
app.delete("/designations/:id", async (req, res) => {
  ...
  const designation = await Designation.findOneAndDelete({ id: parseInt(id) });
  if (!designation) {
    return res.status(404).json({ error: "Designation not found" });
  }
