...
const UserType = require('./models/UserType'); // Adjust the path to your UserType model
...
app.post("/usertypes", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "An error occurred while adding the UserType." });
  }
});
...
app.get("/usertypes", async (req, res) => {
  try {
    // Return in-memory storage as well as MongoDB data
    ...
    res.status(500).json({ error: "An error occurred while fetching the UserTypes." });
  }
});
...
app.put("/usertypes/:id", async (req, res) => {
  ...
  try {
    ...
    const index = userTypes.findIndex(ut => ut._id === id);
    if (index !== -1) {
    ...
    res.status(500).json({ error: "An error occurred while updating the UserType" });
  }
});
...
app.delete("/usertypes/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "UserType deleted successfully", data: result });
  } catch (error) {
    console.error("Error deleting UserType:", error);
    res.status(500).json({ error: "An error occurred while deleting the UserType" });
  }
});
