...
app.put("/userTypes/:id", async (req, res) => {
  ...
  // In-memory update
  const index = userTypes.findIndex((data) => data.id === parseInt(id));
  if (index !== -1) {
    userTypes[index] = updatedUserType;
  }

  // MongoDB update
  const userType = await UserType.findOneAndUpdate({ id: parseInt(id) }, updatedUserType, { new: true });
  if (!userType) {
...
app.delete("/userTypes/:id", async (req, res) => {
  ...
  // MongoDB delete
  const userType = await UserType.findOneAndDelete({ id: parseInt(id) });
  if (!userType) {
    return res.status(404).json({ error: "UserType not found" });
  }
