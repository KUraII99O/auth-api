...
app.get("/routines", async (req, res) => {
  const { userId } = req.query;
  const memoryResults = routineMonitors.filter((routineMonitor) => routineMonitor.userId === userId);
...
app.put("/routines/:id", async (req, res) => {
  ...
  // In-memory update
  const index = routineMonitors.findIndex((routineMonitor) => routineMonitor.id === id);
  if (index !== -1) {
    routineMonitors[index] = updatedRoutineMonitor;
  }

  // MongoDB update
  const routineMonitor = await RoutineMonitor.findOneAndUpdate({ id }, updatedRoutineMonitor, { new: true });
  if (!routineMonitor) {
...
app.post("/vaccines", async (req, res) => {
  const newVaccineMonitor = new VaccineMonitor({ ...req.body });
  vaccineMonitors.push(newVaccineMonitor);

  try {
    await newVaccineMonitor.save();
    res.status(201).json({
      message: "Vaccine monitor added successfully",
      vaccineMonitor: newVaccineMonitor,
    });
  } catch (error) {
    res.status(500).json({ error: "Failed to add vaccine monitor" });
  }
});
...
app.get("/vaccines", async (req, res) => {
  const { userId } = req.query;
  try {
    const monitors = await VaccineMonitor.find({ userId });
    res.json(monitors);
  } catch (error) {
    res.status(500).json({ error: "Failed to fetch vaccine monitors" });
  }
});
...
app.put("/vaccines/:id", async (req, res) => {
  ...
  const index = vaccineMonitors.findIndex(
    (vaccineMonitor) => vaccineMonitor._id.toString() === id
  );
  if (index === -1) {
    return res.status(404).json({ error: "Vaccine monitor record not found" });
  }
  vaccineMonitors[index] = { ...vaccineMonitors[index], ...updatedVaccineMonitor };

  try {
    const monitor = await VaccineMonitor.findByIdAndUpdate(id, updatedVaccineMonitor, { new: true });
    if (!monitor) {
      return res.status(404).json({ error: "Vaccine monitor record not found" });
    }
    res.json({
      message: "Vaccine monitor record updated successfully",
      vaccineMonitor: monitor,
    });
  } catch (error) {
    res.status(500).json({ error: "Failed to update vaccine monitor" });
  }
});
...
app.delete("/vaccines/:id", async (req, res) => {
  ...
  try {
    const monitor = await VaccineMonitor.findByIdAndDelete(id);
    if (!monitor) {
      return res.status(404).json({ error: "Vaccine monitor record not found" });
    }
    res.json({
    ...
    res.status(500).json({ error: "Failed to delete vaccine monitor" });
  }
});
