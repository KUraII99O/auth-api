...
app.put("/cancel/:userId", (req, res) => {
  ...
  // Ensure user has a staff property that is an array
  ...
  // Mark excess staff as read-only if applicable
  if (user.staff.length > freePlan.features.limitations.staff) {
    const excessStaffCount = user.staff.length - freePlan.features.limitations.staff;
    for (let i = 0; i < excessStaffCount; i++) {
      user.staff[i].readOnly = true; // Mark excess staff as read-only
    }
  }
