...
app.post("/cows", async (req, res) => {
  ...
  const cowCount = await Cow.countDocuments({ userId: newCow.userId });
  console.log(`User has ${cowCount} cows`);

  if (user.plan.features.limitations.cows === cowCount) {
...
app.put("/cows/:id", async (req, res) => {
  ...
  const cow = await Cow.findByIdAndUpdate(id, updatedCow, { new: true });
  if (!cow) {
