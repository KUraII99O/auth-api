...
const uri = process.env.MONGODB_URI || "mongodb://localhost:27017"; // Use environment variable or hardcoded URI
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });

const adminhash = bcrypt.hashSync("adminpassword", 10);

// Initialize collections
let usersCollection, staffsCollection, employeesCollection, milksCollection, milksalesCollection, cowFeedsCollection, routineMonitorsCollection, vaccineMonitorsCollection, stallsCollection, expensesCollection, cowsCollection, calvesCollection, pregnanciesCollection, salesCollection, subscriptionPlansCollection, invoicesCollection;
...
client.connect(err => {
  ...
  // Initialize collections
  const db = client.db('dairyFarm');
  usersCollection = db.collection('users');
  staffsCollection = db.collection('staffs');
  employeesCollection = db.collection('employees');
  milksCollection = db.collection('milks');
  milksalesCollection = db.collection('milksales');
  cowFeedsCollection = db.collection('cowFeeds');
  routineMonitorsCollection = db.collection('routineMonitors');
  vaccineMonitorsCollection = db.collection('vaccineMonitors');
  stallsCollection = db.collection('stalls');
  expensesCollection = db.collection('expenses');
  cowsCollection = db.collection('cows');
  calvesCollection = db.collection('calves');
  pregnanciesCollection = db.collection('pregnancies');
...
app.post("/register", async (req, res) => {
  ...
  users.push(newUser);
  await usersCollection.insertOne(newUser); // Save to MongoDB
  ...
  if (plan.id !== 1) {
    ...
    await invoicesCollection.insertOne(newInvoice); // Save to MongoDB
  }
