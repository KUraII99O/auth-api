...
("---------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
// Dummy endpoint to add a user to both in-memory and MongoDB
...
// Dummy endpoint to get users (in-memory version, replace with MongoDB)
app.get('/users', async (req, res) => {
  try {
    const { userId } = req.query;
    let result;
    if (userId) {
      result = users.filter((user) => user.id === userId);
    } else {
...
// Dummy endpoint to edit a user in both in-memory and MongoDB
app.put('/users/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const updatedUser = req.body;

    const updatedUserMongo = await User.findByIdAndUpdate(id, updatedUser, { new: true });

    const index = users.findIndex((user) => user.id === id);
    if (index !== -1) {
      users[index] = { ...users[index], ...updatedUser }; // Update in-memory user (optional)
    }

    res.json({ message: 'User data updated successfully', user: updatedUserMongo });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Dummy endpoint to delete a user from both in-memory and MongoDB
app.delete('/users/:id', async (req, res) => {
  try {
    const { id } = req.params;

    await User.findByIdAndDelete(id); // Delete from MongoDB
...
// Dummy endpoint to toggle user status in both in-memory and MongoDB
app.put('/users/:id/toggle-status', async (req, res) => {
  try {
    ...
    const index = users.findIndex((user) => user.id === id);
    if (index !== -1) {
      users[index].status = user.status; // Toggle in-memory status (optional)
