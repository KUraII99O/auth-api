...
app.post("/pregnancies", (req, res) => {
  ...
  const user = users.find((user) => user.id === newpregnancies.userId);
  if (!user) {
    return res.status(404).json({ error: "User not found" });
  }
  if (
    user.plan.features.limitations.calves ===
    pregnancies.filter((pregnancies) => pregnancies.userId === newCalf.userId).length
  ) {
    return res.status(403).json({ error: "Plan calf limitation reached" });
  }
  calves.push(newCalf);
  res.status(201).json({ message: "Calf added successfully", pregnancies: newpregnancies });
});
