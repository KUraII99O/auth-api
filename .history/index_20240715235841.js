...
app.post("/cowFeeds", async (req, res) => {
  try {
    ...
    cowFeeds.push(newCowFeed); // Add to in-memory storage (optional)
...
app.put("/cowFeeds/:id", async (req, res) => {
  try {
    ...
    const updatedCowFeedMongo = await CowFeed.findOneAndUpdate({ _id: id }, updatedCowFeed, {
      new: true,
    });

    const index = cowFeeds.findIndex((data) => data._id === id);
    if (index !== -1) {
      cowFeeds[index] = updatedCowFeedMongo; // Update in-memory storage (optional)
    }
...
app.delete("/cowFeeds/:id", async (req, res) => {
  try {
    ...
    const deletedCowFeedMongo = await CowFeed.findOneAndDelete({ _id: id });

    let deletedCowFeedInMemory = null;
    const index = cowFeeds.findIndex((data) => data._id === id);
    if (index !== -1) {
      deletedCowFeedInMemory = cowFeeds.splice(index, 1)[0]; // Delete from in-memory storage (optional)
    }

    if (!deletedCowFeedMongo && !deletedCowFeedInMemory) {
      return res.status(404).json({ error: "Cow feed data not found" });
    }

    res.json({
      message: "Cow feed data deleted successfully",
      data: deletedCowFeedMongo || deletedCowFeedInMemory,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
