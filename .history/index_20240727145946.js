...
app.put("/animal-types/:id", async (req, res) => {
  ...
  try {
    // MongoDB update
    const result = await AnimalType.findOneAndUpdate({ _id: id }, updatedAnimalType, { new: true });
    if (!result) {
      return res.status(404).json({ error: "Animal type not found" });
    }

    // Update in-memory storage if needed
    const index = animalTypes.findIndex((animalType) => animalType._id.toString() === id);
    if (index !== -1) {
...
app.delete("/animal-types/:id", async (req, res) => {
  ...
  try {
    // MongoDB delete
    const result = await AnimalType.findOneAndDelete({ _id: id });
    if (!result) {
      return res.status(404).json({ error: "Animal type not found" });
    }
