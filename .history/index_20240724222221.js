...
const expensepurposes = require("./modles/ExpensePurpose"); // Adjusted path
...
app.get("/expense-purposes", async (req, res) => {
  // In-memory retrieval
  res.json(expensePurposes);
  
  // MongoDB retrieval (Uncomment if you want to use MongoDB)
...
app.delete("/expense-purposes/:id", async (req, res) => {
  ...
  if (index !== -1) {
    ...
    // MongoDB delete
    await ExpensePurpose.findOneAndDelete({ id: parseInt(id) });
