...
app.delete("/branches/:id", async (req, res) => {
  ...
  try {
    // MongoDB delete
    const result = await userTypes.findOneAndDelete({ id: id });
    if (!result) {
      return res.status(404).json({ error: "Branch not found" });
    }

    res.json({ message: "user type successfully", data: result });
  } catch (error) {
    console.error("Error deleting user type:", error);
    res.status(500).json({ error: "An error occurred while deleting theuser type" });
  }
});
