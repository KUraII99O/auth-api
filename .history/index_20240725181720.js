...
const userTypes = require("./modles/UserType"); // Adjusted path
...
app.post("/usertypes", async (req, res) => {
  try {
    const newUserType = { ...req.body, createdAt: new Date().toLocaleDateString() };
    ...
    res.status(500).json({ error: "An error occurred while adding the user type." });
  }
});
...
app.get("/usertypes", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "An error occurred while fetching the user types." });
  }
});
...
app.put("/usertypes/:id", async (req, res) => {
  ...
  try {
    ...
    if (index !== -1) {
      userTypes[index] = { ...userTypes[index], ...updatedUserType };
    }
    ...
    res.status(500).json({ error: "An error occurred while updating the user type" });
  }
});
...
app.delete("/usertypes/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "An error occurred while deleting the user type" });
  }
});
