...
app.put("/milks/:id", async (req, res) => {
  try {
    ...
    const updatedMilkMongo = await Milk.findOneAndUpdate({ _id: id }, updatedMilk, {
      new: true,
    });

    const index = milks.findIndex((milk) => milk._id.toString() === id);
    if (index !== -1) {
...
app.delete("/milks/:id", async (req, res) => {
  try {
    ...
    await Milk.findOneAndDelete({ _id: id }); // Delete from MongoDB

    const index = milks.findIndex((milk) => milk._id.toString() === id);
    if (index !== -1) {
      const deletedMilk = milks.splice(index, 1)[0]; // Delete from in-memory storage
      res.json({ message: "Milk deleted successfully", milk: deletedMilk });
    } else {
