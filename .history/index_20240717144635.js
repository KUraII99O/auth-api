...
const Cow = require("./modles/Stalls"); // Adjusted path
...
app.get("/cows", async (req, res) => {
  const { userId } = req.query;
  const dbResults = await Cow.find({ userId: parseInt(userId) });
  res.json(dbResults);
});
...
app.put("/cows/:id", async (req, res) => {
  ...
  // MongoDB update
  const cow = await Cow.findOneAndUpdate({ id: parseInt(id) }, updatedCow, { new: true });
  if (!cow) {
...
app.delete("/cows/:id", async (req, res) => {
  ...
  // MongoDB delete
  const cow = await Cow.findOneAndDelete({ id: parseInt(id) });
  if (!cow) {
    return res.status(404).json({ error: "Cow not found" });
  }
...
app.put("/cows/:id/toggle-status", async (req, res) => {
  ...
  // MongoDB update
  const cow = await Cow.findOne({ id: parseInt(id) });
  if (!cow) {
    return res.status(404).json({ error: "Cow not found" });
  }
