...
app.post("/staffs", async (req, res) => {
  try {
    ...
    // Find the user associated with the staff
    const user = await User.findOne({ id: newStaff.userId });

    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }

    // Count the current number of staff associated with the user
    const currentStaffCount = await Staff.countDocuments({ userId: newStaff.userId });
