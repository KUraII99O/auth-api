...
app.get("/cows", async (req, res) => {
  const { stallNumber,us } = req.query;  // Use req.query to access query parameters

  try {
    // If an id is provided, filter the results
    const dbResults = id ? await Cow.find({ _id: id }) : await Cow.find();
