...
app.put("/branches/:id", async (req, res) => {
  ...
  try {
    // MongoDB update
    const result = await Branch.findOneAndUpdate({ _id: id }, updatedBranch, { new: true });
    if (!result) {
      return res.status(404).json({ error: "Branch not found" });
    }

    // Update in-memory storage if needed
    const index = branches.findIndex((branch) => branch._id.toString() === id);
    if (index !== -1) {
...
app.put("/userTypes/:id", async (req, res) => {
  ...
  try {
    // MongoDB update
    const result = await UserType.findOneAndUpdate({ _id: id }, updatedUserType, { new: true });
    if (!result) {
      return res.status(404).json({ error: "UserType not found" });
    }

    // Update in-memory storage if needed
    const index = userTypes.findIndex((userType) => userType._id.toString() === id);
    if (index !== -1) {
...
app.delete("/userTypes/:id", async (req, res) => {
  ...
  try {
    // MongoDB delete
    const result = await UserType.findOneAndDelete({ _id: id });
    if (!result) {
      return res.status(404).json({ error: "UserType not found" });
    }
...
app.put("/designations/:id", async (req, res) => {
  ...
  try {
    // MongoDB update
    const result = await Designation.findOneAndUpdate({ _id: id }, updatedDesignation, { new: true });
    if (!result) {
      return res.status(404).json({ error: "Designation not found" });
    }

    // Update in-memory storage if needed
    const index = designations.findIndex((designation) => designation._id.toString() === id);
    if (index !== -1) {
...
app.delete("/designations/:id", async (req, res) => {
  ...
  try {
    // MongoDB delete
    const result = await Designation.findOneAndDelete({ _id: id });
    if (!result) {
      return res.status(404).json({ error: "Designation not found" });
    }
...
app.put("/colors/:id", async (req, res) => {
  ...
  try {
    // MongoDB update
    const result = await Color.findOneAndUpdate({ _id: id }, updatedColor, { new: true });
    if (!result) {
      return res.status(404).json({ error: "Color not found" });
    }

    // Update in-memory storage if needed
    const index = colors.findIndex((color) => color._id.toString() === id);
    if (index !== -1) {
      colors[index] = result;
    }
...
app.delete("/colors/:id", async (req, res) => {
  ...
  try {
    // MongoDB delete
    const result = await Color.findOneAndDelete({ _id: id });
    if (!result) {
      return res.status(404).json({ error: "Color not found" });
    }

    // Remove from in-memory storage if needed
    const index = colors.findIndex((colors) => colors._id.toString() === id);
    if (index !== -1) {
      const deletedcolors = colors.splice(index, 1)[0];
      res.json({
