...
app.get("/cows", async (req, res) => {
  const { stallNumber } = req.query;
  console.log(`Received request for stallNumber: ${stallNumber}`);

  if (stallNumber && !mongoose.Types.ObjectId.isValid(stallNumber)) {
    console.log(`Invalid ObjectId format: ${stallNumber}`);
    return res.status(400).json({ message: "Invalid stallNumber format" });
  }

  try {
    const query = stallNumber ? { stallNumber: new mongoose.Types.ObjectId(stallNumber) } : {};
    const dbResults = await Cow.find(query);
    console.log(`Query results: ${JSON.stringify(dbResults)}`);

    if (dbResults.length === 0) {
      console.warn(`No cows found for stallNumber: ${stallNumber}`);
    }
...
app.post("/pregnancies", async (req, res) => {
  const newPregnancy = req.body;

  // Find the user
  const user = users.find((user) => user.id === newPregnancy.userId);
  if (!user) {
    return res.status(404).json({ error: "User not found" });
  }

  // Check if the plan's calf limitation is reached
  const userCalvesCount = pregnancies.filter((pregnancy) => pregnancy.userId === newPregnancy.userId).length;
  if (user.plan.features.limitations.calves === userCalvesCount) {
    return res.status(403).json({ error: "Plan calf limitation reached" });
  }
  ...
  // MongoDB storage
  const pregnancy = new Pregnancy(newPregnancy);
  await pregnancy.save();

  res.status(201).json({ message: "Pregnancy added successfully", data: newPregnancy });
});

app.get("/pregnancies", async (req, res) => {
...
app.put("/pregnancies/:id", async (req, res) => {
  const { id } = req.params;
...
app.delete("/pregnancies/:id", async (req, res) => {
  const { id } = req.params;
