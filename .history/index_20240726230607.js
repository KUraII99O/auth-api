...
app.post("/stalls", async (req, res) => {
  ...
  const newStall = { userId, name, date: new Date().toLocaleDateString() };
...
app.put("/stalls/:id", async (req, res) => {
  ...
  // In-memory update
  const index = stalls.findIndex((data) => data.id === id);
  if (index !== -1) {
    stalls[index] = { ...stalls[index], ...updatedStall };
  }

  // MongoDB update
  const stall = await Stall.findByIdAndUpdate(id, updatedStall, { new: true });
  if (!stall) {
