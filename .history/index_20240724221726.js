...
app.put("/expense-purposes/:id", async (req, res) => {
  ...
  // In-memory update
  const index = expensePurposes.findIndex((data) => data.id === parseInt(id));
  if (index !== -1) {
    expensePurposes[index] = updatedExpensePurpose;
  }

  // MongoDB update
  const expensePurpose = await ExpensePurpose.findOneAndUpdate({ id: parseInt(id) }, updatedExpensePurpose, { new: true });
  if (!expensePurpose) {
...
app.delete("/expense-purposes/:id", async (req, res) => {
  ...
  const index = expensePurposes.findIndex((data) => data.id === parseInt(id));
  if (index !== -1) {
    expensePurposes.splice(index, 1);
  }

  // MongoDB delete
  const expensePurpose = await ExpensePurpose.findOneAndDelete({ id: parseInt(id) });
  if (!expensePurpose) {
    return res.status(404).json({ error: "Expense purpose not found" });
  }

  res.json({ message: "Expense purpose deleted successfully", data: expensePurpose });
});
