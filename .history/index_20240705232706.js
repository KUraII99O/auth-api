...
app.put('/users/:id', async (req, res) => {
  try {
    ...
    const updatedUserMongo = await User.findOneAndUpdate({ id }, updatedUser, { new: true });
...
app.put('/staffs/:id', async (req, res) => {
  ...
  try {
    const updatedStaffMongo = await Staff.findByIdAndUpdate(id, updatedStaff, { new: true });
    const index = staffs.findIndex(staff => staff.id === id);
    if (index !== -1) {
      staffs[index] = updatedStaff; // Update in-memory array
    }
    res.json({ message: 'Staff member data updated successfully', staff: updatedStaffMongo });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.put('/staffs/:id/toggle-status', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Staff member status toggled successfully', staff: staffMember });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
